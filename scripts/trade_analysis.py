...
def main():
    ...
    print(
        f"{'Category':<20} {'Trades':>7} {'Total USDC':>12} {'Avg Size':>10} {'Median':>10} {'Win Rate':>10}"
    )
    print("-" * 75)
    ...
    for cat in sorted(categories, key=lambda c: len(categories[c]), reverse=True):
        ...
        print(
            f"{cat:<20} {count:>7} {total:>12.2f} {avg:>10.2f} {med:>10.2f} {win_rate:>9.1f}%"
        )
    ...
    if btc_trades:
        ...
        for key, trades in sorted(
            by_window.items(), key=lambda x: sum(t["_usdc"] for t in x[1]), reverse=True
        )[:10]:
            total = sum(t["_usdc"] for t in trades)
    ...
    trump_trades = [
        r for r in rows if "Trump" in r["marketName"] or "Xi Jinping" in r["marketName"]
    ]
    if trump_trades:
        ...
        print(
            f"  Trump/Xi cluster: {len(trump_trades)} trades, ${total_trump:.2f} total"
        )
        for m, trades in sorted(
            by_market.items(), key=lambda x: sum(t["_usdc"] for t in x[1]), reverse=True
        ):
            total = sum(t["_usdc"] for t in trades)
    ...
    print(
        f"  Date range: {min(ts_all).strftime('%Y-%m-%d %H:%M')} to {max(ts_all).strftime('%Y-%m-%d %H:%M')} UTC"
    )
