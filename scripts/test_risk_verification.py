...
class TestPositionLimits:
    ...
    def test_max_trade_size_enforced(self, mock_session_cls):
        ...
        assert (
            result.adjusted_size <= 100.0
        ), "Trade size should be capped at MAX_TRADE_SIZE"
    ...
    def test_max_position_fraction_enforced(self, mock_session_cls):
        ...
        assert result.allowed is True
        assert (
            result.adjusted_size <= 80.0
        ), "Position should be capped at 8% of bankroll"
    ...
    def test_max_total_exposure_enforced(self, mock_session_cls):
        ...
        assert result.allowed is True
        assert (
            result.adjusted_size <= 50.0
        ), "Should only allow up to 70% total exposure"
...
class TestDrawdownControls:
    ...
    def test_daily_loss_limit_blocks(self, mock_session_cls):
        ...
        result = rm.validate_trade(
            size=50.0, current_exposure=0.0, bankroll=1000.0, confidence=0.8
        )
