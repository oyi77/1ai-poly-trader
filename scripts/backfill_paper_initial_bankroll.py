...
def calculate_paper_topups(db_session):
    ...
    settings_initial = db_session.execute(
        text("SELECT value FROM settings WHERE key = 'INITIAL_BANKROLL'")
    ).scalar()
    if settings_initial:
    ...
    effective_initial = (
        initial_bankroll if initial_bankroll is not None else settings_initial
    )
...
def apply_corrections(db_session, calc_result, topup_amount=500.0, max_topups=10):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 2. Update paper_pnl
            db_session.execute(
                text("UPDATE bot_state SET paper_pnl = :pnl " "WHERE mode = 'paper'"),
                {"pnl": corrected_pnl},
            )
            ...
            db_session.execute(
                text("UPDATE bot_state SET misc_data = :misc " "WHERE mode = 'paper'"),
            ...
            if "database is locked" in str(e).lower() and attempt < max_retries - 1:
                ...
                print(
                    f"  Database locked, retrying in {delay_ms}ms (attempt {attempt + 1}/{max_retries})"
                )
...
def main():
    ...
    try:
        ...
        print(
            f"  paper_initial_bankroll:   ${result['current_initial_bankroll']:>12,.2f}"
        )
        print(f"  paper_pnl:                ${result['current_pnl']:>12,.2f}")
        print()
        print("CALCULATION:")
        print(
            f"  settings.INITIAL_BANKROLL: ${result['settings_initial_bankroll']:>11,.2f}"
        )
        print(f"  total staked (trade sizes): ${result['total_staked']:>10,.2f}")
        print(f"  total returned:           ${result['total_returned']:>12,.2f}")
        print(
            f"  total auto-topups:         ${result['total_topups']:>10,.2f}  (~{result['estimated_topup_count']} x ${args.topup_amount:,.0f})"
        )
        print()
        print("CORRECTED STATE:")
        print(
            f"  paper_initial_bankroll:   ${result['corrected_initial_bankroll']:>12,.2f}"
        )
        print(f"  paper_pnl:                ${result['corrected_pnl']:>12,.2f}")
        print(
            f"  PnL overstatement:        ${result['pnl_overstatement']:>12,.2f}  (currently counting deposits as profit)"
        )
        print()
        ...
        if not args.apply:
            ...
            print(
                f"  paper_initial_bankroll:   ${verify['current_initial_bankroll']:>12,.2f}"
            )
            print(f"  paper_pnl:                ${verify['current_pnl']:>12,.2f}")
            print(
                f"  topup_count in misc_data: {verify['misc_data'].get('paper_topup_count', 'NOT SET')}"
            )
