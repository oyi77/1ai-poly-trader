...
def main():
    ...
    with engine.connect() as conn:
        ...
        # ── Heartbeats (testnet) ──
        row = conn.execute(
            text("SELECT misc_data FROM bot_state WHERE mode='testnet'")
        ).fetchone()
        ...
        for r in rows:
            ...
            report["strategies"][r[0]] = {
                "trades_7d": r[1],
                "gross_win": round(r[2], 2),
                "gross_loss": round(r[3], 2),
                "pnl_7d": round(r[4], 2),
                "win_rate": round(r[5], 4),
        ...
        cols = [
            ...
            for c in conn.execute(
                text(
                    "SELECT column_name FROM information_schema.columns WHERE table_name = 'bot_state' ORDER BY ordinal_position"
                )
            ).fetchall()
        ]
        ...
        report["wallet_health"]["real_equity"] = (
            round(real_equity, 2) if real_equity is not None else None
        )
        if real_equity is not None:
            drift = round(abs(bot_live_bankroll - real_equity), 2)
            report["wallet_health"]["drift"] = drift
            report["wallet_health"]["drift_pct"] = (
                round(drift / real_equity * 100, 2) if real_equity > 0 else 0
            )
        else:
