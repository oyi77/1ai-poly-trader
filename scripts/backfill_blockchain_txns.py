...
TRANSACTIONS = [
    ...
    # Winning trades (incoming USDC)
    {
        ...
        "from_address": "polymarket",
        "to_address": WALLET_ADDRESS,
        "amount": 4.8992,
        "type": "trade_win",
        "notes": "Winning trade claim",
    },
    {
        "txhash": "win_2",
...
async def backfill_transactions():
    ...
    try:
        ...
        total_deposits = (
            db.execute(
                text(
                    "SELECT SUM(amount) FROM blockchain_transactions WHERE type IN ('deposit', 'trade_win')"
                )
            ).fetchone()[0]
            or 0
        )
        ...
        total_withdrawals = (
            db.execute(
                text(
                    "SELECT SUM(amount) FROM blockchain_transactions WHERE type IN ('withdrawal', 'trade_buy')"
                )
            ).fetchone()[0]
            or 0
        )
