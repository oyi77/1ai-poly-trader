...
def configure_strategies():
    ...
    c.execute(
        "SELECT params FROM strategy_config WHERE strategy_name = 'cex_pm_leadlag'"
    )
    row = c.fetchone()
    if row and row[0]:
        import json
        ...
        c.execute(
            "UPDATE strategy_config SET params = ? WHERE strategy_name = 'cex_pm_leadlag'",
            (json.dumps(params),),
        )
...
def verify():
    ...
    c = conn.cursor()
    c.execute(
        "SELECT strategy_name, enabled, interval_seconds FROM strategy_config ORDER BY strategy_name"
    )
    rows = c.fetchall()
