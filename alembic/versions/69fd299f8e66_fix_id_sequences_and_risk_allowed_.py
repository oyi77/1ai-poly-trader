...
def upgrade():
    ...
    op.execute(
        "ALTER TABLE signals ALTER COLUMN id SET DEFAULT nextval('signals_id_seq')"
    )
...
def downgrade():
    op.execute(
        "ALTER TABLE trade_attempts ALTER COLUMN risk_allowed TYPE double precision USING CASE WHEN risk_allowed THEN 1.0 ELSE 0.0 END"
    )
