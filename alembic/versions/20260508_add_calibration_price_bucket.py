...
def upgrade():
    op.add_column(
        "calibration_records", sa.Column("price_bucket", sa.String(), nullable=True)
    )
    op.create_index(
        "ix_calibration_records_price_bucket", "calibration_records", ["price_bucket"]
    )


def downgrade():
    op.drop_index(
        "ix_calibration_records_price_bucket", table_name="calibration_records"
    )
