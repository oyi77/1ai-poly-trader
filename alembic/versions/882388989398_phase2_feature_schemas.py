...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_strategy_proposal_strategy_name", "strategy_proposal", ["strategy_name"]
    )
    op.create_index(
        "ix_strategy_proposal_created_at", "strategy_proposal", ["created_at"]
    )
    ...
    try:
        op.add_column(
            "audit_log",
            sa.Column("user_id", sa.String(), nullable=True, server_default="system"),
        )
