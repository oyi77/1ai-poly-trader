...
async def get_brain_status(db: Session = Depends(get_db)):
    ...
    mirofish_enabled = (
        str(mirofish_setting.value).lower() in ("true", "1", "yes")
        if mirofish_setting
        else False
    )
...
async def get_debate_transcript(
    ...
    if not decision.signal_data:
        raise HTTPException(
            status_code=404, detail="No debate data available for this decision"
        )
    ...
    if not debate_data:
        raise HTTPException(
            status_code=404, detail="No debate transcript in signal data"
        )
    ...
    return DebateTranscript(
        ...
        consensus_probability=debate_data.get("judge", {}).get(
