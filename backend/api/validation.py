...
class TradeCreateRequest(BaseModel):
    ...
    platform: Platform = Field(
        default=Platform.polymarket, description="Trading platform"
    )
    ...
    strategy_name: Optional[str] = Field(
        None, max_length=100, description="Strategy that generated this trade"
    )
    confidence: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="Signal confidence (0.0-1.0)"
    )
    reasoning: Optional[str] = Field(
        None, max_length=5000, description="Trade reasoning or notes"
    )
...
class SignalCreateRequest(BaseModel):
    ...
    market_id: str = Field(
        ..., min_length=1, max_length=500, description="Market identifier"
    )
    prediction: float = Field(
        ..., ge=0.0, le=1.0, description="Predicted probability (0.0-1.0)"
    )
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Signal confidence (0.0-1.0)"
    )
    reasoning: str = Field(
        ...,
        min_length=10,
...
class StrategyConfigRequest(BaseModel):
    ...
    interval_seconds: Optional[int] = Field(
        None, ge=10, le=86400, description="Strategy execution interval (10s - 24h)"
    )
    trading_mode: Optional[TradingMode] = Field(
        None, description="Trading mode override for this strategy"
    )
...
class WalletConfigCreateRequest(BaseModel):
    ...
    address: str = Field(
        ..., min_length=42, max_length=42, description="Ethereum wallet address (0x...)"
    )
    pseudonym: Optional[str] = Field(
        None, max_length=100, description="Wallet nickname"
    )
    ...
    tags: Optional[List[str]] = Field(
        None, max_length=20, description="Wallet tags (max 20)"
    )
    enabled: Optional[bool] = Field(
        default=True, description="Enable wallet for trading"
    )
...
class WalletConfigUpdateRequest(BaseModel):
    ...
    tags: Optional[List[str]] = Field(None, max_length=20, description="Wallet tags")
    enabled: Optional[bool] = Field(None, description="Enable/disable wallet")
    notes: Optional[str] = Field(None, max_length=2000, description="Wallet notes")
...
class BacktestRunRequest(BaseModel):
    ...
    strategy_name: str = Field(
        ..., min_length=1, max_length=100, description="Strategy to backtest"
    )
    start_date: Optional[str] = Field(
        None, description="Backtest start date (ISO format)"
    )
    end_date: Optional[str] = Field(None, description="Backtest end date (ISO format)")
    initial_bankroll: float = Field(
        default=10000.0, gt=0, le=10000000, description="Initial bankroll ($0-$10M)"
    )
    kelly_fraction: float = Field(
        default=0.25, ge=0.01, le=1.0, description="Kelly fraction (0.01-1.0)"
    )
    max_trade_size: float = Field(
        default=1000.0, gt=0, le=100000, description="Max trade size ($0-$100K)"
    )
    ...
    daily_loss_limit: float = Field(
        default=500.0, gt=0, le=100000, description="Daily loss limit ($0-$100K)"
    )
...
class ProposalCreateRequest(BaseModel):
    ...
    strategy_name: str = Field(
        ..., min_length=1, max_length=100, description="Strategy name"
    )
    change_details: Dict[str, Any] = Field(
        ..., description="Proposed configuration changes"
    )
    expected_impact: float = Field(
        ..., ge=-1.0, le=1.0, description="Expected impact on performance (-1.0 to 1.0)"
    )
...
class ProposalApprovalRequest(BaseModel):
    ...
    admin_user_id: str = Field(
        ..., min_length=1, max_length=100, description="Admin user identifier"
    )
    reason: str = Field(
        ..., min_length=1, max_length=2000, description="Approval/rejection reason"
    )
...
class CredentialsUpdateRequest(BaseModel):
    ...
    private_key: Optional[str] = Field(
        None, min_length=64, max_length=66, description="Private key (64-66 hex chars)"
    )
    ...
    api_passphrase: Optional[str] = Field(
        None, max_length=500, description="API passphrase"
    )
    ...
    def validate_at_least_one_field(self):
        ...
        if not any(
            [self.private_key, self.api_key, self.api_secret, self.api_passphrase]
        ):
