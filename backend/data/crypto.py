...
async def fetch_crypto_klines(
    ...
    # Background REST refresh happens via cache TTL expiry
    if cache["data"] is not None:
        cache_age = now - cache["ts"]
        if cache_age < _CACHE_TTL * 3:  # use stale up to 3x TTL
...
async def _fetch_klines_with_fallback(
    ...
    if kraken_breaker.state != "OPEN":
        try:
            ...
            logger.warning(
                f"Kraken kline fetch failed for {pair}, trying Binance: {repr(e)}"
            )
    ...
    try:
        ...
        rows = list(reversed(rows))
        candles = [[int(r[0]), r[1], r[2], r[3], r[4], r[5]] for r in rows]
        cache["data"] = candles
        cache["ts"] = now
        cache["_source"] = "bybit"
...
def _build_fallback_microstructure(
    asset: str = "bitcoin", source: str = "fallback"
...
async def compute_crypto_microstructure(
    ...
    try:
        ...
        typical_prices = [
            (highs[-i] + lows[-i] + closes[-i]) / 3 for i in range(1, vwap_window + 1)
        ]
        vwap_volumes = [volumes[-i] for i in range(1, vwap_window + 1)]
        total_vol = sum(vwap_volumes)
        if total_vol > 0:
            vwap = (
                sum(tp * v for tp, v in zip(typical_prices, vwap_volumes)) / total_vol
            )
        else:
            vwap = current_price
        ...
        sma_crossover = (
            (sma5 - sma15) / current_price * 100 if current_price > 0 else 0.0
        )
...
def estimate_price_probability(
    current_price: float, threshold: float, direction: str, volatility_24h: float = 0.05
) -> float:
    """
