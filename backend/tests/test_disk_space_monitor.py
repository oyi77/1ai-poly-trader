...
class TestDiskSpaceCheck:
    def test_healthy_when_below_threshold(self, checker):
        ...
        with patch("shutil.disk_usage", return_value=fake_usage):
            with patch(
                "backend.core.agi_health_check.settings",
                SimpleNamespace(
    ...
    def test_unhealthy_when_above_threshold(self, checker):
        fake_usage = SimpleNamespace(
            used=95 * (1024**3), total=100 * (1024**3), free=5 * (1024**3)
        )
        with patch("shutil.disk_usage", return_value=fake_usage):
            with patch(
                "backend.core.agi_health_check.settings",
            ...
            with patch(
                ...
                with patch("backend.config.ROOT_DIR", "/tmp"):
                    with patch("os.path.exists", return_value=True):
                        with patch(
                            "os.path.getsize", return_value=200 * 1024 * 1024
                        ):  # 200 MB
                            result = checker._check_disk_space()
        assert result["healthy"] is False
...
class TestDiskSpaceInRunChecks:
    def test_disk_space_included_in_results(self, checker):
        with patch.object(checker, "_check_strategies", return_value={"healthy": True}):
            with patch.object(
                checker, "_check_data_freshness", return_value={"healthy": True}
            ):
                with patch.object(
                    checker, "_check_budget", return_value={"healthy": True}
                ):
                    with patch.object(
                        checker, "_check_scheduler", return_value={"healthy": True}
                    ):
                        with patch.object(
                            checker,
                            ...
                            with patch.object(
                                checker,
                                "_check_disk_space",
                                return_value={"healthy": True, "used_pct": 50.0},
                            ):
