...
def create_test_genome(stage="DRAFT", **metrics_kwargs) -> StrategyGenome:
    ...
    metrics = FitnessMetrics(
        ...
        capital_rotation_efficiency=metrics_kwargs.get(
...
def test_should_promote_paper_to_live():
    ...
    genome = create_test_genome(
        stage="PAPER", total_trades=49, sharpe_ratio=0.6, win_rate=0.6
    )
...
def test_draft_to_shadow_transition():
    ...
    genome = create_test_genome(stage="DRAFT", sharpe_ratio=0.35, max_drawdown_pct=0.20)
    ...
    genome = create_test_genome(stage="DRAFT", sharpe_ratio=0.25, max_drawdown_pct=0.20)
...
def test_shadow_to_paper_transition():
    ...
    genome = create_test_genome(stage="SHADOW", win_rate=0.65)  # High signal accuracy
...
def test_live_to_breeding_transition():
    ...
    genome = create_test_genome(stage="LIVE", max_drawdown_pct=0.55, total_trades=100)
...
def test_breeding_to_legend_transition():
    ...
    with patch.object(manager, "_get_stage_entered_at") as mock_stage:
        with patch.object(manager, "_get_total_pnl_for_genome") as mock_pnl:
            ...
            result = manager.evaluate_stage_transition(
                genome_no_legend, "volatile", MagicMock()
            )
            assert result is None
...
def test_breeding_to_live_downgrade():
    ...
    genome = create_test_genome(
        stage="BREEDING", sharpe_ratio=1.5, win_rate=0.60, total_trades=100
    )
...
def test_check_auto_kill():
    ...
    genome = create_test_genome(max_drawdown_pct=0.55)
    ...
    genome = create_test_genome(sharpe_ratio=-2.5, win_rate=0.04)
    ...
    # Test no kill
    genome = create_test_genome(sharpe_ratio=0.5, win_rate=0.55)
...
def test_execute_transition():
    ...
    mock_db.query.return_value.filter.return_value.first.return_value = (
        mock_genome_registry
    )

    with patch(
        "backend.application.agi.lifecycle_manager.publish_event"
    ) as mock_publish:
...
def test_graveyard_rehabilitation():
    ...
    with patch(
        "backend.application.agi.lifecycle_manager.check_rehabilitation_eligibility"
    ) as mock_check:
