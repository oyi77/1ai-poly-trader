...
def provider():
    with patch.dict(
        os.environ,
        {"WALLET_PRIVATE_KEY": _VALID_KEY, "ASTER_PRIVATE_KEY": _VALID_KEY},
        clear=False,
    ):
        with patch(
            "backend.clients.aster_client.AsterClient.__init__", return_value=None
        ):
...
async def test_get_balance(provider):
    ...
    mock_client.get_balance = AsyncMock(
        return_value={"USDC": {"free": "1000", "total": "1500", "used": "500"}}
    )
    provider._client = mock_client
