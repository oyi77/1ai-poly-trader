...
def test_execute_proposal_success(
    executor, db_session, approved_proposal, sample_strategy_config
):
    ...
    audit_log = (
        ...
        .filter(AuditLog.event_type == "PROPOSAL_EXECUTED")
        .first()
    )
...
def test_execute_proposal_already_executed(
    executor, db_session, sample_strategy_config
):
    proposal = StrategyProposal(
        strategy_name="test_strategy",
        change_details={"min_edge": 0.08},
...
def test_measure_impact_positive(executor, db_session, executed_proposal):
    ...
    create_trades(
        db_session, "test_strategy", 5, execution_time + timedelta(hours=1), after_pnls
    )
...
def test_measure_impact_not_enough_trades(executor, db_session, executed_proposal):
    ...
    create_trades(
        db_session, "test_strategy", 2, execution_time + timedelta(hours=1), [5, 10]
    )
...
def test_auto_rollback_negative_impact(
    executor, db_session, executed_proposal, sample_strategy_config
):
    execution_time = executed_proposal.executed_at
    ...
    rollback_log = (
        db_session.query(AuditLog)
        .filter(AuditLog.event_type == "PROPOSAL_ROLLED_BACK")
        .first()
    )
...
@pytest.mark.asyncio
async def test_execute_approved_proposals_job(
    db_session, approved_proposal, sample_strategy_config
):
    proposal_id = approved_proposal.id
...
@pytest.mark.asyncio
async def test_measure_impact_and_rollback_job(
    db_session, executed_proposal, sample_strategy_config
):
    execution_time = executed_proposal.executed_at
    proposal_id = executed_proposal.id
    ...
    with patch("backend.db.utils.SessionLocal", return_value=db_session):
        with patch(
            "backend.core.proposal_executor.ProposalExecutor.get_executed_proposals"
        ) as mock_get:
