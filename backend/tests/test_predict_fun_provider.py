"""Test suite for PredictFunProvider."""
...
def provider():
    with patch.dict(os.environ, {"AZURO_GRAPH_URL": "https://graph.azuro.org", "AZURO_RPC_URL": "https://rpc.azuro.org"}, clear=False):
        ...
        return PredictFunProvider(paper_mode=True)
...
async def test_place_order_paper_mode(provider):
    ...
    assert result.filled_avg_price == Decimal("0.6")
...
async def test_get_markets(provider):
    ...
    mock_client.get_markets = AsyncMock(return_value=[
        {"conditionId": "c1", "question": "Will ETH hit 5k?"},
        {"conditionId": "c2", "title": "BTC 200k"},
    ])
    provider._client = mock_client
    markets = await provider.get_markets()
    assert len(markets) == 2
    assert markets[0].venue == "predict_fun"
    assert markets[0].market_id == "c1"
    assert markets[0].title == "Will ETH hit 5k?"
...
async def test_place_order_live_success(provider):
    provider._paper_mode = False
    mock_client = AsyncMock()
    mock_client.sign_and_send_bet = AsyncMock(return_value="0xtx456")
    provider._client = mock_client
    order = NormalizedOrder(
        market_id="cond1", side=OrderSide.BUY,
        order_type=OrderType.MARKET, size=Decimal("10"),
    ...
    with patch.dict(os.environ, {"AZURO_PRIVATE_KEY": "0xkey"}, clear=False):
        result = await provider.place_order(order)
    assert result.venue_order_id == "0xtx456"
    assert result.status == OrderStatus.FILLED
...
async def test_place_order_live_exception(provider):
    provider._paper_mode = False
    mock_client = AsyncMock()
    mock_client.sign_and_send_bet = AsyncMock(side_effect=Exception("revert"))
    provider._client = mock_client
    order = NormalizedOrder(
        market_id="cond1", side=OrderSide.BUY,
        order_type=OrderType.MARKET, size=Decimal("10"),
