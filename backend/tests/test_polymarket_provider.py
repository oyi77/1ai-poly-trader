...
@pytest.fixture
def provider():
    with patch.dict(os.environ, {"POLYMARKET_BUILDER_API_KEY": "k", "POLYMARKET_BUILDER_SECRET": "s"}, clear=False):
...
async def test_place_order_paper_mode(provider):
    order = NormalizedOrder(
        ...
        order_type=OrderType.LIMIT, size=Decimal("50"),
        price=Decimal("0.65"),
    )
...
def test_resolve_token_id_yes_side(provider):
    order = NormalizedOrder(
        market_id="cond1", side=OrderSide.YES,
...
def test_resolve_token_id_no_side(provider):
    order = NormalizedOrder(
        market_id="cond1", side=OrderSide.NO,
        order_type=OrderType.LIMIT, size=Decimal("1"),
        metadata={"yes_token_id": "tok_yes", "no_token_id": "tok_no"},
    )
...
def test_resolve_token_id_fallback_to_market_id(provider):
    order = NormalizedOrder(
        market_id="cond_fallback", side=OrderSide.BUY,
        order_type=OrderType.LIMIT, size=Decimal("1"),
    )
...
def test_rejected_helper(provider):
    order = NormalizedOrder(
        market_id="c", side=OrderSide.BUY,
...
async def test_search_markets_filters_category(provider):
    mock_gamma = [
        {"condition_id": "a", "question": "Crypto?", "category": "crypto", "outcomePrices": "[0.5]", "volume": 1, "active": True},
        {"condition_id": "b", "question": "Econ?", "category": "economics", "outcomePrices": "[0.5]", "volume": 1, "active": True},
    ]
    import backend.data.gamma as gamma_mod
    with patch.object(gamma_mod, "fetch_markets", return_value=mock_gamma):
...
async def test_get_positions_with_mock_clob():
    ...
    mock_clob.get_trader_positions = AsyncMock(return_value=[
        {"market_id": "m1", "outcome": "YES", "size": 100, "avg_price": 0.65, "current_price": 0.70, "unrealized_pnl": 5.0},
