...
class TestActivitySourceInterfaces:
    ...
    SOURCES = [
        (
            ...
            "AsterActivitySource",
            {"wallet_address": "0xtest", "client": MagicMock()},
        ),
        (
            "hyperliquid",
        ...
        (
            ...
            {"wallet_address": "0xtest", "ws_client": MagicMock()},
        ),
        (
            "polymarket",
            "backend.core.activity.sources.polymarket_source",
            "PolymarketActivitySource",
            {"wallet_address": "0xtest", "clob_client": MagicMock()},
        ),
        (
            "azuro",
...
class TestClientMethods:
    ...
    CLIENTS = [
        (
            ...
            ["get_fills", "get_balance", "get_positions"],
        ),
        (
            "OstiumClient",
            "backend.clients.ostium_client",
            ["get_fills", "get_balance", "get_positions", "health_check"],
        ),
        (
            "MyriadClient",
            "backend.clients.myriad_client",
            ["get_fills", "get_balance", "get_positions", "health_check"],
        ),
        (
            "SXBetClient",
            "backend.clients.sxbet_client",
            ["get_fills", "get_balance", "get_positions", "health_check"],
        ),
        # ("LimitlessClient", "backend.clients.limitless_client", ["get_fills", "health_check"]),  # DISABLED
        (
            "AzuroClient",
            "backend.clients.azuro_client",
            ["cached_query", "health_check"],
        ),
    ]

    @pytest.mark.parametrize(
        "cls_name,module,methods", CLIENTS, ids=[c[0] for c in CLIENTS]
    )
...
class TestOrchestratorRegistration:
    ...
    def test_all_sources_registered(self):
        ...
        for name in expected:
            assert (
                f'register_source("{name}"' in source
