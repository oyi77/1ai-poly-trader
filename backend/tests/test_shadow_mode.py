...
class TestDBSessionShadowRunner:
    ...
    def test_settle_calculates_pnl_and_accuracy(self):
        ...
        runner.settle(
            market_ticker=unique_ticker, settlement_value=1.0, actual_outcome=0.55
        )
    ...
    def test_performance_aggregation(self):
        ...
        runner.settle(
            "BTC-DOWN-67890", settlement_value=1.0
        )  # Loss (down but settled at 1.0)
        ...
        assert (
            abs(perf.avg_edge) < 1e-6
        )  # Average of (model_prob - entry_price) should be close to 0
    ...
    def test_promotion_eligibility_accuracy_above_threshold(self):
        ...
        runner.settle(
            "BTC-UP-2001", settlement_value=1.0, actual_outcome=0.63
        )  # Within 0.2 of 0.65
        ...
        runner.settle(
            "BTC-UP-2002", settlement_value=1.0, actual_outcome=0.58
        )  # Within 0.2 of 0.60
