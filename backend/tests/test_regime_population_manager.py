...
def test_detect_regime_and_rebalance():
    ...
    with patch(
        "backend.application.agi.regime_population_manager._build_market_data"
    ) as mock_data:
        mock_data.return_value = {"prices": list(range(100))}
        with patch(
            "backend.application.agi.regime_population_manager.RegimeDetector"
        ) as mock_detector:
            with patch(
                "backend.application.agi.regime_population_manager.regime_changed",
                return_value=True,
            ):
                with patch(
                    "backend.application.agi.regime_population_manager.publish_event"
                ) as mock_publish:
                    mock_detector.return_value.detect_regime.return_value.regime.value = (
                        "volatile"
                    )
    ...
    with patch(
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "backend.application.agi.regime_population_manager.increase_archetype_allocation"
                ) as mock_increase:
                    with patch(
                        "backend.application.agi.regime_population_manager.decrease_archetype_allocation"
                    ) as mock_decrease:
                        regime = detect_regime_and_rebalance(db)
