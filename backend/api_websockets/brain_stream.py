...
async def broadcast_signal_received(signal_data: Dict[str, Any]):
    ...
    if tm:
        await tm.create_task(
            topic_manager.broadcast("brain", message), name="broadcast_signal_received"
        )
    else:
...
async def broadcast_debate_started(market_id: str, nodes: list):
    ...
    if tm:
        await tm.create_task(
            topic_manager.broadcast("brain", message), name="broadcast_debate_started"
        )
    else:
        asyncio.create_task(topic_manager.broadcast("brain", message))
...
async def broadcast_debate_ended(market_id: str, consensus: float, confidence: float):
    ...
    if tm:
        await tm.create_task(
            topic_manager.broadcast("brain", message), name="broadcast_debate_ended"
        )
    else:
        asyncio.create_task(topic_manager.broadcast("brain", message))
...
async def broadcast_trade_executed(trade_data: Dict[str, Any]):
    ...
    if tm:
        await tm.create_task(
            topic_manager.broadcast("brain", message), name="broadcast_trade_executed"
        )
    else:
        asyncio.create_task(topic_manager.broadcast("brain", message))
...
async def broadcast_proposal_generated(proposal_data: Dict[str, Any]):
    ...
    if tm:
        await tm.create_task(
            topic_manager.broadcast("brain", message),
            name="broadcast_proposal_generated",
        )
    else:
        asyncio.create_task(topic_manager.broadcast("brain", message))
    logger.debug(
        f"Queued proposal_generated broadcast: {proposal_data.get('strategy_name')}"
    )
...
async def broadcast_node_status_change(node_id: str, status: str):
    ...
    if tm:
        await tm.create_task(
            topic_manager.broadcast("brain", message),
            name="broadcast_node_status_change",
        )
    else:
        asyncio.create_task(topic_manager.broadcast("brain", message))
...
async def broadcast_edge_activation(from_node: str, to_node: str, active: bool):
    ...
    if tm:
        await tm.create_task(
            topic_manager.broadcast("brain", message), name="broadcast_edge_activation"
        )
    else:
        asyncio.create_task(topic_manager.broadcast("brain", message))
    logger.debug(
        f"Queued edge_activation broadcast: {from_node} -> {to_node} (active={active})"
    )
