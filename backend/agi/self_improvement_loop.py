...
class SelfImprovementEngine:
    ...
    async def run_pipeline(
        ...
        try:
            ...
            logger.info(
                "[SelfImprovement] Found {} improvement candidates", len(candidates)
            )

            if not candidates:
                cycle.completed_at = time.time()
                cycle.duration_ms = round(
            ...
            if dry_run:
                logger.info(
                    "[SelfImprovement] DRY RUN — would address {} candidates",
                    len(to_address),
                )
                for c in to_address:
                    logger.info(
            ...
            if actions:
                logger.info(
                    "[SelfImprovement] {} recovery actions triggered", len(actions)
                )
            ...
            if cycle.changes_merged > 0:
                ...
                logger.info(
                    "[SelfImprovement] Health score: {} → {}",
                    cycle.health_score_before,
                    cycle.health_score_after,
                )
        ...
        logger.info(
            "[SelfImprovement] Cycle {} complete in {:.1f}s — {} merged, {} abandoned",
            cycle.cycle_id,
            cycle.duration_ms / 1000,
            cycle.changes_merged,
            cycle.changes_abandoned,
        )
        return cycle
    ...
    def _record_health_snapshot(self, candidates: list[ImprovementCandidate]) -> Any:
    ...
    async def run_scheduled(
        ...
        self._running = True
        logger.info(
            "[SelfImprovement] Scheduled loop started ({}h interval)", interval_hours
        )
        ...
        while self._running:
            ...
            logger.info(
                "[SelfImprovement] Sleeping for {}h until next cycle", sleep_hours
            )
            ...
            "recent_cycles": [
                {
                    ...
                    "health_delta": (
                        ...
                        if c.health_score_after is not None
                        else None
                    ),
