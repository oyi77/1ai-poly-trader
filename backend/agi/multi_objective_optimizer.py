...
class MultiObjectiveOptimizer:
    ...
    def optimize_allocation(self, goals: List[Goal]) -> Dict[str, float]:
        ...
        try:
            ...
            return self._fallback_allocation(
                goals, self.daily_bankroll_cap, self.domain_diversification_limit
            )
        except Exception:
            return self._fallback_allocation(
                goals, self.daily_bankroll_cap, self.domain_diversification_limit
    ...
    def _optimize(self, goals: List[Goal]):
        n_goals = len(goals)
        initial_guess = np.ones(n_goals) * min(
            self.daily_bankroll_cap / max(1, n_goals), self.domain_diversification_limit
        )
        bounds = [(0, self.domain_diversification_limit) for _ in range(n_goals)]
        constraints = [
            {"type": "ineq", "fun": lambda x: self.daily_bankroll_cap - np.sum(x)}
        ]
        return minimize(
    ...
    def _objective_function(self, x, goals: List[Goal]) -> float:
        ...
        for i, goal in enumerate(goals):
            ...
            goal_utility += self.objective_weights[
                "time_horizon_penalty"
            ] * self._time_horizon_penalty(goal.time_horizon)
            total_utility += allocations[i] * goal_utility
        return -total_utility

    def _time_horizon_penalty(self, time_horizon: str) -> float:
        return {"short_term": 0.1, "medium_term": 0.3, "long_term": 0.6}.get(
            time_horizon, 0.3
        )

    def _process_optimization_result(
        self, result, goals: List[Goal]
    ) -> Dict[str, float]:
        return {goal.goal_id: float(result.x[i]) for i, goal in enumerate(goals)}

    def _fallback_allocation(
        self, goals: List[Goal], daily_cap: float, domain_limit: float
    ) -> Dict[str, float]:
        if not goals:
            return {}
        n_goals = len(goals)
    ...
    def get_health_metrics(
        self, allocations: Dict[str, float], goals: List[Goal]
    ) -> Dict[str, float]:
        return {
            "allocation_efficiency": 0.5,
            "risk_concentration": 0.5,
            "time_diversification": 0.5,
        }
