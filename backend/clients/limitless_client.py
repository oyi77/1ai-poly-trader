...
class LimitlessClient:
    ...
    async def place_order(
        ...
        raise RuntimeError(
            "LimitlessClient.place_order: EIP-712 signing not implemented — orders cannot be placed without cryptographic signatures"
        )

    async def cancel_order(self, order_id: str, private_key: str) -> bool:
        """Cancel an open order. EIP-712 signing not implemented."""
        raise RuntimeError(
            "LimitlessClient.cancel_order: EIP-712 signing not implemented — orders cannot be cancelled without cryptographic signatures"
        )
