...
class TradeForensics:
    ...
    def _diagnose(
        self, trade: Trade, signal: Optional[Any], db: Session
    ) -> Dict[str, Any]:
        ...
        if trade.entry_price and trade.market_ticker:
            try:
                ...
                if signal_record and signal_record.price:
                    slippage = (
                        abs(trade.entry_price - signal_record.price)
                        / signal_record.price
                        if signal_record.price > 0
                        else 0
                    )
                    if slippage > 0.02:
    ...
    def _get_suggestions(self, root_cause: str) -> List[str]:
        mapping = {
            "low_confidence_signal": [
                "Raise AUTO_APPROVE_MIN_CONFIDENCE",
                "Review prompt engineering",
            ],
            "strategy_degrading": [
                "Consider disabling strategy temporarily",
    ...
    async def analyze_recent_losses(self, lookback_hours: int = 24) -> Dict[str, Any]:
        ...
        with get_db_session() as db:
            ...
            all_factors = [
                f for a in analyses for f in a.get("contributing_factors", [])
            ]
