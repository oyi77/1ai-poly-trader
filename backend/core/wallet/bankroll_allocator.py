        ...
        try:
            ...
            for mode in settings.active_modes_set:
                with get_db_session() as db:
                    state = for_update(
                        db, db.query(BotState).filter_by(mode=mode)
                    ).first()
                    if not state:
                        continue
                    ...
                    if bankroll <= 0:
                        logger.warning(
                            f"[BankrollAllocator] {mode}: Bankroll ${bankroll:.2f} too low, skipping"
                        )
                        continue

                    allocations = self.ranker.auto_allocate(
                        db, bankroll, lookback_days=30, trading_mode=mode
                    )
                    ...
                    try:
                        ...
                        logger.exception(
                            "[BankrollAllocator] Failed to parse BotState.misc_data JSON"
                        )
                        misc = {}
                    ...
                    db.commit()
                    logger.info(
                        f"[BankrollAllocator] {mode}: Persisted allocations to BotState"
                    )
                    all_allocations[mode] = allocations

            if not all_allocations:
                logger.warning(
                    "[BankrollAllocator] No BotState found for any active mode, skipping allocation"
                )
                return {}
        ...
        try:
            ...
            for strategy, bias in category_bias.items():
                if bias < -0.05:
                    ...
                    logger.info(
                        f"[BankrollAllocator] Longshot discount {strategy}: bias={bias:.4f}"
                    )
            ...
            with get_db_session() as db:
                ...
                for strat, strat_trades in by_strategy.items():
                    ...
                    if maker_trades and taker_trades:
                        maker_avg = sum(t.pnl or 0 for t in maker_trades) / len(
                            maker_trades
                        )
                        taker_avg = sum(t.pnl or 0 for t in taker_trades) / len(
                            taker_trades
                        )
                        if maker_avg > taker_avg * 1.1:
                            maker_boost_strategies.add(strat)
                            logger.info(
                                f"[BankrollAllocator] Maker edge for {strat}: maker={maker_avg:.4f} vs taker={taker_avg:.4f}"
                            )
    ...
    def apply_calibration_feedback(
        ...
        try:
            ...
            biased_buckets = {
                b["bucket"]
                for b in buckets
                if b.get("bias_direction") == "overconfident"
            }
            ...
            if biased_buckets:
                logger.info(
                    f"[BankrollAllocator] Overconfident buckets detected: {biased_buckets}"
                )
...
def get_wallet_allocation(
    self, strategy_name: str, total_allocation: float
) -> dict[int, float]:
    from backend.db.utils import get_db_session
    ...
    try:
        ...
        logger.error(
            f"[BankrollAllocator] get_wallet_allocation failed for {strategy_name}: {e}"
        )
        return {}
