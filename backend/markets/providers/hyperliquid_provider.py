...
class HyperliquidProvider(BaseMarketProvider):
    ...
    async def get_balance(self) -> NormalizedBalance:
        """Get account balance."""
        state = await self._client.get_balance()
        margin_summary = state.get("marginSummary", {})
        return NormalizedBalance(
            venue="hyperliquid",
            available_cash=Decimal(str(margin_summary.get("accountValue", "0"))),
            total_equity=Decimal(str(margin_summary.get("accountValue", "0"))),
            reserved_margin=Decimal(str(margin_summary.get("totalMarginUsed", "0"))),
            currency="USDC",
            raw=state,
        )
