...
export function EquityChart({ data, initialBankroll }: Props) {
  ...
  // Optimization: Use .reduce() instead of spread operator to avoid Maximum call stack size exceeded errors
  ...
  // Optimization: Single-pass .reduce() for bankroll min/max without intermediate array allocation
  const minBankroll = data.reduce((min, d) => Math.min(min, d.bankroll ?? initialBankroll), initialBankroll)
  const maxBankroll = data.reduce((max, d) => Math.max(max, d.bankroll ?? initialBankroll), initialBankroll)
