...
def draw_scene_title(fig, ax, progress):
    ...
    # Title
    draw_text_centered(
        ax, DATA["title"], 0.62, fontsize=80, color=ACCENT, weight="bold", alpha=alpha
    )
    draw_text_centered(
        ax,
        DATA["subtitle"],
    ...
    for i in range(5):
        ...
        ax.plot(
            ...
            [y, y],
            color=ACCENT,
            alpha=line_alpha * (1 - abs(i - 2) * 0.2),
            linewidth=0.5,
            transform=ax.transAxes,
        )
    ...
    if progress > 0.5:
        ...
        for i, (stage, scolor) in enumerate(zip(stages, stage_colors)):
            ...
            if i < 3:
                ax.annotate(
                    "",
                    xy=(0.15 + (i + 1) * 0.22 - 0.05, 0.22),
                    xytext=(0.15 + i * 0.22 + 0.05, 0.22),
                    transform=ax.transAxes,
                    arrowprops=dict(
                        arrowstyle="->", color=ACCENT, lw=1.5, alpha=flow_alpha
                    ),
                )
...
def draw_scene_call(fig, ax, progress):
    ...
    draw_text_centered(
        ax,
        "github.com/berkah-karya/polyedge",
        0.48,
        fontsize=24,
        color=WHITE,
        weight="normal",
...
anim = FuncAnimation(
    fig, render_frame, frames=total_frames, blit=False, interval=1000 / FPS
)
