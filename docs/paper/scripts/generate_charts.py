...
def generate_performance_charts():
    ...
    ax1.axhline(
        10000, color="gray", linestyle="--", alpha=0.5, label="Starting Bankroll"
    )
    ax1.set_ylabel("Equity (USDC)")
    ax1.set_title(
        "PolyEdge Live Deployment: Equity Curve & Cumulative P&L", fontweight="bold"
    )
    ...
    ax2.bar(
        trade_nums, cumulative, color=colors, width=1.0, edgecolor="none", alpha=0.7
    )
...
def generate_experiments_chart():
    ...
    counts = [
        int(m["experiments_backtest"]),
        int(m["experiments_draft"]),
        int(m["experiments_shadow"]),
    ]
    colors = ["#3498db", "#f39c12", "#2ecc71"]
    bars = ax.barh(
        stages, counts, color=colors, edgecolor="black", linewidth=1.0, height=0.5
    )
...
def generate_strategy_distribution():
    ...
    counts = [
        int(m["experiments_general_scanner"]),
        int(m["experiments_unknown"]),
        int(m["experiments_btc_oracle"]),
        4,
    ]
