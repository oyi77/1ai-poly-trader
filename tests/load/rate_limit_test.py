...
class RateLimitTester:
    ...
    async def test_endpoint_at_limit(
    ...
    def verify_rate_limit_headers(self, headers: Dict, config: RateLimitConfig) -> bool:
        ...
        try:
            ...
            if limit == config.limit:
                ...
                logger.error(
                    f"  ✗ Limit value incorrect: {limit} (expected {config.limit})"
                )
                all_present = False
            ...
            if reset > time.time():
                logger.info(
                    f"  ✓ Reset timestamp valid: {reset} ({reset - time.time():.1f}s from now)"
                )
            else:
    ...
    async def test_reset_after_timeout(
        self, config: RateLimitConfig, wait_seconds: int = 65
    ) -> bool:
        """
        ...
        logger.info(
            f"  Waiting {wait_seconds} seconds for rate limit window to reset..."
        )
        ...
        try:
            async with httpx.AsyncClient(timeout=TIMEOUT) as client:
                ...
                if response.status_code in [200, 401]:
                    logger.info(
                        f"  ✓ Request succeeded after reset: {response.status_code}"
                    )
                    ...
                    logger.error(
                        f"  ✗ Request failed after reset: {response.status_code}"
                    )
                    return False
        except Exception as e:
    ...
    async def test_multi_ip_independent_limits(self) -> bool:
        ...
        for ip, result in results.items():
            if (
                result["successful"] >= result["expected_limit"] - 2
                    ...
                    if retry_after:
                        ...
                        try:
                            ...
                            if 0 < retry_seconds <= 60:
                                logger.info(
                                    f"  ✓ Retry-After value valid: {retry_seconds}s"
                                )
                                return True
                            else:
                                logger.error(
                                    f"  ✗ Retry-After value out of range: {retry_seconds}s"
                                )
                                return False
                        except ValueError:
                            logger.error(
                                f"  ✗ Retry-After value not an integer: {retry_after}"
                            )
                            return False
                    else:
    ...
    async def run_all_tests(self, skip_reset_test: bool = True) -> Dict:
        ...
        summary = {"endpoints": {}, "multi_ip": None, "all_passed": True}
        ...
        for config in ENDPOINTS:
            ...
            # Test at limit
            successful, rate_limited, headers = await self.test_endpoint_at_limit(
                config
            )

            at_limit_passed = (
                successful >= config.limit - 2  # Allow small variance
                and rate_limited >= 1
            )
    ...
    def print_summary(self, summary: Dict):
        ...
        for endpoint, results in summary["endpoints"].items():
            ...
            logger.info(
                f"  {retry_status} Retry-After Header: {results['retry_after']}"
            )
