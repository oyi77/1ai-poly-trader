    ...
    async def connect(self):
        ...
        try:
            ...
            await self.websocket.send(
                json.dumps({"action": "subscribe", "topic": self.endpoint})
            )
...
class LoadTestRunner:
            ...
            for i in range(num_to_churn):
                if i < len(self.clients):
                    ...
                    new_client = WebSocketClient(
                        i, self.endpoint, self.base_url, self.token
                    )
    ...
    async def run(self):
        ...
        background_tasks = [asyncio.create_task(self.monitor_resources())]
        ...
        return self.generate_report(
            initial_cpu, initial_memory, final_cpu, final_memory
        )
    ...
    def generate_report(
        ...
        connected_clients = sum(
            1 for c in self.clients if c.connected or c.messages_received > 0
        )
        ...
        report = {
            ...
            "message_stats": {
                ...
                "messages_per_client": (
                    total_messages / self.num_clients if self.num_clients > 0 else 0
                ),
                "messages_per_second": (
                    total_messages / self.duration if self.duration > 0 else 0
                ),
            },
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        print(
            f"  Connected: {conn['connected']}/{report['test_config']['num_clients']}"
        )
        ...
        if all_ok:
            ...
            if not latency_ok:
                print(
                    f"  - Latency p99 >= 200ms ({report['latency_stats']['p99']:.2f}ms)"
                )
            if not errors_ok:
                print(f"  - Errors detected ({err['total_errors']} errors)")
            if not cpu_ok:
                print(
                    f"  - CPU exceeded 80% (Max: {res.get('max_cpu_percent', 0):.1f}%)"
                )
            if not memory_ok:
                print(
                    f"  - Memory exceeded 2GB (Max: {res.get('max_memory_gb', 0):.2f}GB)"
                )
        print(f"{'='*70}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="WebSocket Load Test for TopicWebSocketManager"
    )
    parser.add_argument(
        "--clients",
        type=int,
    ...
    parser.add_argument(
        "--churn",
        action="store_true",
        help="Enable connection churn test (clients connect/disconnect)",
    )
    parser.add_argument(
        "--broadcast-rate",
        type=int,
        default=0,
        help="Target broadcast messages per second (0 = natural rate)",
    )
    parser.add_argument(
        "--endpoint",
        type=str,
    ...
    parser.add_argument(
        "--token", type=str, default="", help="Admin API token for authentication"
    )
    parser.add_argument(
