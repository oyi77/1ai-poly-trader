...
class TestMETARCoordinates:
    ...
    def test_unit_values_valid(self):
        ...
        from backend.data.weather import CITY_CONFIG
        for city_key, config in CITY_CONFIG.items():
            assert config["unit"] in ("F", "C"), f"City {city_key} has invalid unit: {config['unit']}"

    def test_us_cities_use_fahrenheit(self):
        """US cities (nyc, chicago, miami, etc.) must use Fahrenheit."""
        from backend.data.weather import CITY_CONFIG
        us_cities = ["nyc", "chicago", "miami", "dallas", "seattle", "atlanta", "los_angeles", "denver"]
...
class TestCalibration:
    ...
    def test_welford_update(self, tmp_path):
        ...
        try:
            ...
            for _ in range(25):
                cal_module.update_calibration("nyc", "gefs", forecast_temp_f=80.0, actual_temp_f=77.0)
            ...
            assert sigma < 1.0, f"Sigma should be near 0 for constant errors, got {sigma:.3f}"
...
class TestEdgeCalculation:
    ...
    def test_kelly_size_basic(self):
        ...
        assert size <= 50.0, "Kelly should not exceed 5% of $1000 bankroll"

    def test_kelly_size_capped_at_five_percent(self):
        """Kelly never exceeds 5% of bankroll regardless of edge."""
        from backend.core.signals import calculate_kelly_size
        ...
        assert size <= 500.0, f"Kelly should be capped at 5% of $10000, got ${size:.2f}"
