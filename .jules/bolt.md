...
**Action:** Use `.reduce()` such as `arr.reduce((max, x) => Math.max(max, x.val), -Infinity)` instead to avoid creating temporary mapped arrays and hitting call stack limits.
