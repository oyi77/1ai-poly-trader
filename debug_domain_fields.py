...
async def debug_domain():
    ...
    async with clob:
        try:
            ...
            ds = utils_builder.domain_separator
            print(f"Domain Name: {ds.name}")
            print(f"Domain Version: {ds.version}")
            print(f"Domain ChainId: {ds.chainId}")
            print(f"Domain VerifyingContract: {ds.verifyingContract}")
